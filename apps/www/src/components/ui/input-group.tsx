...
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
	return (
		// biome-ignore lint/a11y/useSemanticElements: Using div with role=group for flexible input composition
		<div
			className={cn(
...
function InputGroupAddon({
	...
	return (
		// biome-ignore lint/a11y/noStaticElementInteractions: This div is an addon container with legitimate mouse interaction for input focus management
		<div
...
function InputGroupText({ className, ...props }: React.ComponentProps<"span">) {
	return (
		<span
			className={cn(
				"line-clamp-1 flex items-center gap-2 text-muted-foreground in-[[data-slot=input-group]:has([data-slot=input-control],[data-slot=textarea-control])]:[&_svg:not([class*='size-'])]:size-4.5 sm:in-[[data-slot=input-group]:has([data-slot=input-control],[data-slot=textarea-control])]:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:-mx-0.5",
				className,
			)}
			{...props}
		/>
